...
import { UserType, UserTypeAccount, UserTypeAuth, UserTypeInformation, UserTypeRequest } from "../utils/interfaces/UserType";
...
export class UserModel {
 ...
 async show(userId: number): Promise<UserTypeInformation> {
  try {
    ...
    const sql = "SELECT user_id, user_first_name, user_last_name FROM users WHERE user_id = $1";
  ...
  async create(user: UserTypeRequest): Promise<UserTypeAuth> {
    try {
        // @ts-ignore
        const conn = await client.connect();
        const sql = "INSERT INTO users (user_first_name, user_last_name, user_password) VALUES ($1, $2, $3) RETURNING *";
  ...
  async update(user: UserType): Promise<UserTypeInformation> {
    try {
        // @ts-ignore
        const conn = await client.connect();
        const sql = "UPDATE users SET user_first_name = $1, user_last_name = $2, user_password = $3 WHERE user_id = $4 RETURNING *";
        const result = await conn.query(sql, [
          user.first_name, 
          user.last_name, 
          user.password, 
      ...
      try {
        ...
        if(result.rows.length) {
          ...
          if (bcrypt.compareSync(user.password + PEPPER, userRes.user_password)) {
            const auth: UserTypeAuth = {user_id: user.user_id, token: generateToken(userRes)}
            return auth;
          }
        }
